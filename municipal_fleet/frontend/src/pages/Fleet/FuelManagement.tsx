            ...
            <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} title="Abastecimento">
                <form onSubmit={handleSave} className="data-form">
                    <div className="data-form-grid">
                        ...
                        <label>Produto <select value={form.product || ""} onChange={e => setForm({ ...form, product: Number(e.target.value) })}><option value="">Selecione</option>{products.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
